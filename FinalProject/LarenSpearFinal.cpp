...
class Person
{
    ...
    public:
        ...
        string status_string() //healthy unvaccinated = 0, recovered = -1, vaccinated = -2, sick n days to recovery
        {
            if (status == 0)
            {
                return "susceptible";
...
class Population
{
    ...
    public:
        ...
        Population(int x, int n_interactions, float transmission, int days_sick, float percentvaccinated) //population size, interactions, transmission probability, days sick, %vax
        {
            for(int i = 0; i < x; i++)
            {
                Person j;
...
int body(int sickdays, int popsize, float transmission, int n_interactions, float percentvaccinated)
{
//    Person joe;
//    int step = 1;
...
//        }
//    }
    Population myPop(popsize,transmission,n_interactions,sickdays,percentvaccinated); //population size, interactions, transmission probability, days sick, %vax
    ...
    outfile << popsize << " " << transmission << " " << n_interactions << " " << total_days << " " << peak  << " " << percentvaccinated << endl; //add more stuff
    outfile.close();
...
int main()
{
    ...
    for(int j=0; j < 5; j++)
    {
        ...
        for (float k = 0; k <=1.0; k = k + 0.1) //less than 5% of population get sick
        {
